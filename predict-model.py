...
def run_code():
	...
	print("prediction is calculated to: {0:0d}".format(np.argmax(digit[0])))
	print("class is calculated to: {0:1s}".format(class_names[np.argmax(digit[0])]))
	print("prediction estimated to: {0:4.2f}%".format(100*np.max(digit)))
	#print("Array:", digit)
